...
const logPath = path.join(outputRoot, 'public-record-translation.log')
const progressPath = path.join(outputRoot, 'public-record-translation-progress.json')
const model = String(process.argv.find((arg) => arg.startsWith('--model='))?.slice('--model='.length) || 'translategemma:12b')
