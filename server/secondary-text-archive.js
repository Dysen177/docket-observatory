...
export async function syncSecondaryTextArchive(options = {}) {
  ...
  if (!archive) throw new Error('The local secondary text archive is unavailable.')
  const payload = sanitizeArchivePayload(archive)
  await writeCompressedArchive(outputPath, payload)
  activeArchivePromise = Promise.resolve(payload)
  return payload
}

export async function retrieveSecondaryArchiveEvidence(query, tokens = [], language = 'zh', limit = 4, options = {}) {
  ...
  return candidates
    ...
    .map(({ record, detail, score }) => ({
      ...
      subtitle: english
        ? `Local secondary text archive · ${archiveKindLabel(record.docKind, 'en')}`
        : `本地辅助文字档案 · ${archiveKindLabel(record.docKind, 'zh')}`,
      date: record.publishedDate ?? record.addedAt?.slice(0, 10) ?? null,
      timestamp: null,
      pageNumber: null,
      sourceUrl: null,
      sourceLabel: english ? 'Local secondary archive' : '本地辅助档案',
...
async function loadPreferredArchive() {
  ...
  if (!runtime) return bundled ? sanitizeArchivePayload(bundled) : null
  if (!bundled) return sanitizeArchivePayload(runtime)
  return sanitizeArchivePayload(Date.parse(runtime.fetchedAt ?? '') >= Date.parse(bundled.fetchedAt ?? '') ? runtime : bundled)
}
...
function sanitizeArchiveRecord(record) {
  const versions = Array.isArray(record.versions)
    ? record.versions.map((version) => ({ ...version, sourceUrl: null, downloadUrl: null }))
    : []
  const details = Object.fromEntries(Object.entries(record.details ?? {}).map(([language, detail]) => [
    language,
    { ...detail, sourceUrl: null, downloadUrl: null },
  ]))
  return { ...record, versions, details }
}

function sanitizeArchivePayload(archive) {
  const records = archive.records
    .filter((record) => record.slug !== 'site-guide-navigation')
    .map(sanitizeArchiveRecord)
  return {
    schemaVersion: archiveSchemaVersion,
    source: {
      name: 'Local Secondary Text Archive',
    ...
    sync: {
      ...
      refreshedRecords: 0,
      fetchedDetails: 0,
      retainedDetails: records.reduce((total, record) => total + Object.keys(record.details ?? {}).length, 0),
      failures: [],
    },
  }
}
...
export async function secondaryTextArchiveFileInfo() {
