...
`v0.1.1` 是完整社区版：安装包完整携带截至 2026 年 8 月 15 日整理完成的公开发布基线、搜索索引和既有双语研究辅助，不是首次启动后再慢慢下载历史资料的轻量客户端。“完整”指完整携带本次公开发布基线，不表示已经取得 PACER 中的密封、受限、撤下或尚未公开材料。当前提供 macOS 和 Windows 桌面版，不提供 iPhone、iPad 或 Android 版本。

| 电脑 | 下载文件 | 大小 |
| --- | --- | ---: |
| Apple 芯片 Mac（Apple silicon） | [下载 macOS arm64 DMG](https://github.com/Dysen177/docket-observatory/releases/download/v0.1.1/Docket-Observatory-0.1.1-macOS-arm64-unsigned.dmg) | 约 1.84 GB |
| Intel Mac | [下载 macOS x64 DMG](https://github.com/Dysen177/docket-observatory/releases/download/v0.1.1/Docket-Observatory-0.1.1-macOS-x64-unsigned.dmg) | 约 1.85 GB |
| Windows 10/11 64 位 | [下载 Windows x64 EXE](https://github.com/Dysen177/docket-observatory/releases/download/v0.1.1/Docket-Observatory-0.1.1-Windows-x64-unsigned.exe) | 约 1.69 GB |
...
免费未签名表示没有购买 Apple Developer ID 或 Windows 商业发布者证书，不表示安装包未经测试。`v0.1.1` 的两份 DMG 已通过映像完整性、只读挂载、应用资源封装、目标架构和 PDF 原生依赖检查；Windows EXE 已在真实 GitHub Windows 环境完成下载、NSIS 校验、安装、完整资料核对、界面启动、运行中覆盖升级、升级后再次启动和卸载。macOS 与 Windows 源码构建、搜索、密钥加密和无 Key 默认流程也已通过。

- [Windows 原生安装验证记录](https://github.com/Dysen177/docket-observatory/actions/runs/31884932315)
- [macOS / Windows 跨平台源码验证记录](https://github.com/Dysen177/docket-observatory/actions/runs/31885225295)
...
| 历史公开言论 | 独立索引 2017 年 1 月 26 日至 2023 年 3 月 14 日的 2,523 条可访问直播/视频来源记录，并内置 3,187 条历史视频目录和 3,063 份可检索原语言文字稿。选中条目后，右侧直接显示本地已收录的全部文字，无需再打开弹窗；关键词可跨多场直播检索，每场命中都独立保留。界面分开标注长时直播、剪辑/短视频和 239 条可能不完整的公开副本，并说明开头、结尾或文字量缺口；另有 124 条明确标记为暂无正文。 |
| AI 全库研究对话 | 左侧独立的 AI 聊天工作区可同时检索法院 PDF、翻译与法律解读、直播文字、案件时间线、实体和政策资料，再由用户配置的 OpenAI、Claude、Gemini、兼容中转站或 Ollama 生成带证据编号的回答。没有可用模型时，聊天输入会明确停用，不会把普通检索结果伪装成 AI 判断。 |
...
- 1,865 条资料记录；
- 2,523 条去重后的历史直播/公开视频记录，覆盖 2017-01-26 至 2023-03-14；其中 2,406 条另有经核验备用副本；
- 3,187 条历史视频目录、3,063 份可全文检索文字稿和逐段时间点；文字正文、搜索索引与外部原视频链接随完整版安装包提供，不保存第三方图片、视频或音频；
- 1,838 份有效 PDF，共 1,570,950,358 字节；
- 1,795 份按文件内容去重后的 PDF 已全部纳入全文索引，其中 1,770 份正文提取完整、25 份受扫描质量或原始结构限制而仅能部分提取；
...
- 154 个案卷已进入关系归属审计：76 个有公开证据确认、20 个属于较高概率关联、39 个排除、19 个等待更多公开证据；待核验案卷及其文件仍完整保留并可搜索；
...
| AI 全库研究对话 | 聊天功能停用；案卷和直播的独立全文检索仍可正常使用 | 配置任一受支持模型后，可对整个本地资料库多轮提问；回答分开法院认定、诉讼主张、公开言论和政策背景，并附原始证据 |
