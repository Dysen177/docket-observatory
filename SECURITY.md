...
- Downloaded court files and mutable caches are ignored by git, but complete DMG/EXE releases include a separately staged, sanitized, read-only court-file baseline and a precomputed research seed. `release:prepare-data` excludes settings, diagnostics, automation history, logs, and developer-local paths. Internal corpus and seed-cache manifests are generated under `release-metadata/` for the build and audit process; they are not user download attachments. The packaged app verifies every seed file before installing it into writable per-user storage; failure stops startup rather than silently rebuilding an empty or lower-quality historical baseline.
...
- No-token RECAP access is limited to public Atom feeds for fixed tracked docket IDs and returns metadata only. Token-enhanced RECAP downloads are limited to those tracked dockets and `storage.courtlistener.com` PDF paths returned by the API; downloaded PDFs receive a local integrity record.
...
Publishing source code does not prove that an arbitrary DMG/EXE was built from that source. The current zero-cost community release is explicitly labeled `unsigned`; its source, build workflow, complete-data manifests, and internal release validation remain available in the repository. A future formally signed tier additionally requires Apple Developer ID signing and notarization for macOS and trusted timestamped Authenticode for Windows. GitHub Actions are pinned to full commit SHAs; Dependabot, CodeQL, dependency review, the lockfile, and `npm audit` reduce supply-chain risk but cannot eliminate it.

Users should download binaries only from the official GitHub Release and avoid unreviewed forks or third-party repackaging. The repository owner should require two-factor authentication, branch protection, reviewed pull requests, private vulnerability reporting, and least-privilege release secrets.
